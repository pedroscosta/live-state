...
- **Delete `collectionRoute`/`Route`.** Routes become procedure-only (`ProcedureRoute`). A resource is queryable by virtue of being in the `schema`, not because a route is declared for it. The query engine resolves Tracked Queries directly against `batcher`/storage keyed by `resource`, dropping the `router.get → routes[resource].handleQuery` indirection.
